...
inline void PrintSOF(const unsigned char *stream) {
	...
	const char *nr_components_to_string[] = {"????", "Grayscale", "????", "YCbCr", "CYMK"};
...
inline int ParseDQT(stJpegData *jdata, const unsigned char *stream) {
	...
	while (length > 0) {
		...
		if (qindex > 4) {
			Serial.print("Error - No more 4 quantization table is supported (got ");
			Serial.print(qi);
			Serial.println(")");
		}
...
inline int ParseSOS(stJpegData *jdata, const unsigned char *stream) {
	...
	*/
	...
	for (unsigned int i = 0; i < nr_components; i++) {
		...
		// Serial.print("ComponentId:%d  tableAC:%d tableDC:%d\n", cid, table & 0xf, table >> 4);
...
inline int ParseDHT(stJpegData *jdata, const unsigned char *stream) {
	...
	*/
	...
	// Serial.print("> DHT marker (length=%d)\n", length);
	...
	while (length > 0) {
		...
		if ((index & 0xf) >= HUFFMAN_TABLES) {
			Serial.print("Error - No mode than 4 Huffman tables is supported\n");
		}
		// Serial.print("Huffman table %s n%d\n", (index & 0xf0) ? "AC" : "DC", index & 0xf);
		// Serial.print("Length of the table: %d\n", count);
...
inline int ParseJFIF(stJpegData *jdata, const unsigned char *stream) {
	...
	while (!sos_marker_found) {
		...
		default: {
			// Serial.print("ERROR> Unknown marker %2.2x\n", marker);
		} break;
		}
...
inline int JpegParseHeader(stJpegData *jdata, const unsigned char *buf, unsigned int size) {
	...
	// Serial.print("-|- File thinks its size is: %d bytes\n", fileSize);
...
void Jpeg::renderAt(const Coords &coords, const shader_t &shader) const {
	...
	if (!jdec) {
		...
		if (JpegParseHeader(jdec, bytes, length) < 0) {
	...
	Serial.println((unsigned long)bytes);
	...
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			...
			auto pixeldata = jdec->m_rgb + (y * jdec->m_width + x) * 3;

			color_t pixel = rgb(pixeldata[0], pixeldata[1], pixeldata[2]);
			if (shader) {
				pixel = shader(pixel, {x, y}, {jdec->m_width, jdec->m_height}, now);
			}
...
// 	// Assuming SOF0. Support for more JPEG encoding logic may come in future.
// 	// For now, only support this since it's the most common.
// 	// const size_t headerLength = getInt(startOfFrame, 2);
// 	const uint8_t precision = getInt(startOfFrame + 2, 1);
// 	const size_t rows = getInt(startOfFrame + 3, 2);
// 	const size_t cols = getInt(startOfFrame + 5, 2);
// 	// const uint8_t imageComponents = getInt(startOfFrame + 7, 1);

// 	const auto startOfScan = afterMarker(0xd8, startOfFrame + 8);
// 	if (!startOfScan) {
// 		// Error in the data, no "Start of Scan" data section.
// 		return;
// 	}

// 	// Image header data
// 	const size_t scanHeaderLength = getInt(startOfScan, 2);
// 	const uint8_t components = bytes[startOfScan + 2];
// 	const uint8_t componentSelector = bytes[startOfScan + 3];
// 	const uint8_t dcCodingTableSelector = (bytes[startOfScan + 4] & 0xf0) >> 4;
// 	const uint8_t acCodingTableSelector = bytes[startOfScan + 4] & 0x0f;
// 	const uint8_t spectralSelectionStart = bytes[startOfScan + 5];
// 	const uint8_t spectralSelectionEnd = bytes[startOfScan + 6];
// 	const uint8_t successiveApproxHi = (bytes[startOfScan + 7] & 0xf0) >> 4;
// 	const uint8_t successiveApproxLo = bytes[startOfScan + 7] & 0x0f;
